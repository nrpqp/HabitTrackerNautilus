...
**Reason**: La escala pasa de cuatro a cinco niveles numerados de 1 a 5, con un peldaño nuevo entre Lite y Estándar que emite partículas con presupuesto reducido. Además, la detección deja de ejecutarse en cada arranque: pasa a sembrar el valor inicial una sola vez, porque desaparece la posición automática de la preferencia.

**Migration**: Sustituido por «Clasificación del dispositivo en cinco niveles», que define la escala, y por «Semilla del nivel en el primer arranque», que define la derivación desde las señales del navegador. El escenario de `prefers-reduced-motion` se traslada al requisito «Techo permanente por movimiento reducido», donde deja de ser una entrada de la detección y pasa a sujetar el nivel activo en todo momento. Correspondencia de niveles: `0 → 1`, `1 → 2`, `2 → 4`, `3 → 5`.
