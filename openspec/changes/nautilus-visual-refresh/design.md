...
- **El cambio de fuente descuadra los labels de la rueda**, que se dibujan dentro del SVG con la fuente heredada → Es el único punto donde la tipografía toca la rueda sin tocar su código. Space Grotesk es algo más ancha que Outfit; comprobar con nombres de 15 caracteres (el máximo) que el label no se sale del arco. Si se sale, la mitigación es bajar el `font-size` del label **por CSS**, sin tocar `svg.js`.
